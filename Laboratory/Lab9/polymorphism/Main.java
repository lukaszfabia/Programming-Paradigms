...
public class Main {
    static Elf[] workers = new Elf[] { new CasualElf("TONY"), new IcicleElf("ANTHONY"), new MixedElf("DONALD") };
    ...
    public static void letsGo(List<Bauble> baubles) {
        for (Bauble bauble : baubles) {
            ...
            if (!passedToColleague) {
                System.out.println(bauble.name + ": has been destroyed by " + workers[i - 1].getName() + "!");
            }
        }
    }
}
